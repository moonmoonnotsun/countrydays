#!/usr/bin/env node
/**
 * Generate localized landing pages for countrydays.app
...
// concatenated title fragments (or around an inline app-name link) would be
// visually wrong for these locales. All linking helpers below check this set
// so no accidental space is inserted next to punctuation-free CJK text.
...
function titleSep(code, left, right) {
  ...
  // continuation) - that would read wrong regardless of locale.
...
// Concatenates before/link/after text directly (no automatic spacing).
// Correct spacing for every locale - including whether a space, no space
// (CJK), or a leading particle should touch the app-name link - is already
// baked into the translated `before`/`after` strings in landing-phrases.json,
// so this stays a straight string join rather than guessing with regexes.
function linkedText(before, appName, after, storeUrl) {
  return `${before || ''}${appLink(storeUrl, appName)}${after || ''}`;
}

// The hero tagline always starts with the app name link followed by the
// tagline sentence. Non-CJK locales need a space after the link; CJK locales
// attach the following particle directly to the link with no space.
function taglineText(code, appName, tagline, storeUrl) {
  const link = appLink(storeUrl, appName);
  return NO_SPACE_LOCALES.has(code) ? `${link}${tagline}` : `${link} ${tagline}`;
}
...
function buildLocalePage(code) {
  ...
  html = html.replace(
    /https:\/\/countrydays\.app\/assets\//g,
    `${BASE_URL}/assets/`,
  );
  html = html.replace(
  ...
  html = html.replace(/"inLanguage": "en-US"/, `"inLanguage": "${o.htmlLang}"`);
  html = html.replace(/"url": "https:\/\/countrydays\.app\/"/g, `"url": "${pageUrl}"`);
  html = html.replace(
    /"name": "Country Days Tracker & Counter"/,
    `"name": "${escapeJson(appName)}"`,
  );
  html = html.replace(
    /"description": "Country days tracker for iPhone - count days in every country, follow Schengen 90\/180 and tax residency rules, with GPS auto-tracking, photo import, and on-device privacy\."/,
  ...
  html = html.replace(
    ...
    `<h1 class="app-title">${o.heroH1Before}${titleSep(code, o.heroH1Before, o.heroH1Highlight)}<span class="highlight-brand">${o.heroH1Highlight}</span>${titleSep(code, o.heroH1Highlight, o.heroH1After)}${o.heroH1After}</h1>`,
  );
  html = html.replace(
    /<p class="app-tagline">[\s\S]*?<\/p>/,
    `<p class="app-tagline">${taglineText(code, appName, o.heroTagline, storeUrl)}</p>`,
  );

  html = html.replace(
    /<span class="title-line">Visa compliance,<\/span>\s*<span class="title-line title-accent">tax residency, zero guesswork<\/span>/,
    `<span class="title-line">${o.sectionFeaturesLine1}</span>\n                    <span class="title-line title-accent">${o.sectionFeaturesLine2}</span>`,
  );
  html = html.replace(
    /<p class="section-subtitle">Everything you need to track days abroad\. From Schengen limits to 183-day tax rules, all in one clean app\.<\/p>/,
  ...
  html = html.replace(
    /<p class="section-subtitle">Clean interface built for travelers, nomads, and expats<\/p>/,
    `<p class="section-subtitle">${o.sectionScreenshotsSubtitle}</p>`,
  );

  html = html.replace(
    /<h2 class="section-title">What is a <span class="title-accent">country days tracker<\/span>\?<\/h2>/,
  ...
  html = html.replace(
    /<h2 class="section-title">Country Days <span class="title-accent">FAQ<\/span><\/h2>/,
  ...
  html = html.replace(
    /<p class="section-subtitle">Common questions about our country days tracker for iPhone<\/p>/,
  ...
  html = html.replace(/aria-label="Download Country Days Tracker & Counter on the App Store"/g, `aria-label="${o.downloadAppStore}"`);
  html = html.replace(/<span class="logo-text">Country Days Tracker & Counter<\/span>/, `<span class="logo-text">${appName}</span>`);
...
function updateSitemap() {
  ...
  const urls = ALL_LOCALES.map(({ code, path: p }) => {
    const altLinks = [
      ...ALL_LOCALES.map(
